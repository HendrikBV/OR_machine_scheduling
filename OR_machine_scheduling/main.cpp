...
class IPModelAlt : public Algorithm
{
	CPXENVptr env = nullptr;
	CPXLPptr problem = nullptr;
...
void IPModelAlt::run_explanation()
{
	_output = true;
...
void IPModelAlt::initialize_cplex()
{
	int status = 0;
	char error_text[CPXMESSAGEBUFSIZE];
...
void IPModelAlt::build_problem()
{
	...
	// write to file
	status = CPXwriteprob(env, problem, "IPModelAlt.lp", NULL);
	if (status != 0)
	{
		CPXgeterrorstring(env, status, error_text);
		throw std::runtime_error("Error in function IPModel::build_problem(). \nCouldn't write problem to lp-file. \nReason: " + std::string(error_text));
...
public:
	static std::unique_ptr<Algorithm> create(const std::string& algorithm)
	{
		...
		else if (algorithm == "IPAlt" || algorithm == "integer programming alternative")
			return std::make_unique<IPModelAlt>();
		else
