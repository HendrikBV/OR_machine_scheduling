	...
	try
	{
		...
		options.add_options()
			("algorithm", "The choice of algorithm. Possibilities:"
				...
				"\n\t* \"IP1\": an integer programming model (with variables x[j][k] = 1 if job j in position k in the sequence, 0 otherwise)"
				"\n\t* \"IP2\": an alternative integer programming model (with variables z[i][j] = 1 if job i comes before job j in the sequence, 0 otherwise)"
